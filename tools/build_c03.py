# ============================================================
# build_c03.py — 세라핀(c03) 전투 원화 13장 변환 (c02 파이프라인)
#
#   assets/sprites/_source_c03/*.png  →  assets/sprites/c01/*.png (192×128)
#
...
SRC = os.path.join(ROOT, 'assets', 'sprites', '_source_c03')
OUT = os.path.join(ROOT, 'assets', 'sprites', 'c03')

W, H = 192, 128
FOOT_ROW = 120
CHAR_H = 120            # 날개 성장 머리가 기준 높이에 포함돼 몸이 작아진다 → 보정 상향 (몸 실측 ≈ 주인공 -10%)
THICK = 8
MIN_CC = 40            # 이보다 작은 잉크 조각은 배경 노이즈로 제거

# 오른쪽을 보는 원본 — 왼쪽으로 반전 (줌 시트 육안 + 출력 검수로 확정)
# 전원 오른쪽 보기 — 인게임 실측으로 확정 (2026-08-22 방향 반대 신고).
# ★금발 함정: 살색 픽셀 중심 판별이 금발(살색 필터에 걸림)에 속아 머리채 쪽을 얼굴로 읽었다.
#   금발·갈발 캐릭터는 살색 자동 판별을 믿지 말고 인게임 방향으로 검증할 것.
