# ============================================================
# build_skeleton_archer.py — 해골 궁수 전투 원화 10장 변환 + 2장 합성 (적 2호)
#
#   assets/sprites/_source_skeleton_archer/*.png  →  assets/sprites/c01/*.png (192×128)
#
...
SRC = os.path.join(ROOT, 'assets', 'sprites', '_source_skeleton_archer')
OUT = os.path.join(ROOT, 'assets', 'sprites', 'skeleton_archer')

W, H = 192, 128
FOOT_ROW = 120
CHAR_H = 116            # 활 포함 실측 절충 — 62는 활 끝이 다수 잘리고 54는 몸이 작다
THICK = 8
...
# 이 세트는 10장 발주분: idle1(호흡)·hurt(젖힘)는 idle0에서 합성, cast는 등록표에서 attack1 재사용
NAMES = ['idle0', 'walk0', 'walk1', 'walk2', 'walk3',
         'attack0', 'attack1', 'attack2', 'attack3', 'down']
...
def synth():
    # idle1 = idle0 상체 1px 가라앉힘 (field_sprites bobFrame과 같은 원리)
    ...
    # hurt = idle0 상체를 뒤(오른쪽)로 기울임 — 왼쪽 보기 기준 젖혀지는 방향
    hurt = np.zeros_like(im)
    for y in range(H):
        sh = max(0, int(round((92 - y) * 0.22))) if y < 92 else 0
        if sh:
            hurt[y, sh:] = im[y, :W - sh]
        else:
            hurt[y] = im[y]
    Image.fromarray(hurt, 'RGBA').save(os.path.join(OUT, 'hurt.png'))
    print('합성: idle1(호흡), hurt(젖힘)')
