...
export class PlaceholderScene extends Scene {
  ...
  buttons() {
    // 4개가 되면서 250~710(폭 460)에 안 들어간다 — 170~790으로 넓히고 폭을 145로 줄였다
    return [
      { id: 'field', label: '탐험 나가기 [F]', x: 170, y: 430, w: 145, h: 36 },
      { id: 'battle', label: '출격 편성 [B]', x: 327, y: 430, w: 145, h: 36 },
      { id: 'save', label: '저장하기 [S]', x: 484, y: 430, w: 145, h: 36 },
      { id: 'title', label: '타이틀로 [ESC]', x: 641, y: 430, w: 145, h: 36 },
    ];
  }

  goField(game) {
    game.audio.playSE('confirm');
    game.changeScene('field');   // 이어서 탐험 (저장된 맵·좌표에서 재개)
  }

  goBattle(game) {
    game.audio.playSE('confirm');
    game.changeScene('party');   // 편성 → 전투 시작
  }
  ...
  onKeyDown(code, game) {
    ...
    else if (code === 'KeyF') this.goField(game);
  ...
  onMouseDown(x, y, button, game) {
    ...
    for (const b of this.buttons()) {
      if (inRect(x, y, b)) {
        ...
        else if (b.id === 'field') this.goField(game);
  ...
  render(g, game) {
    ...
    drawText(g, 'M3 — [출격 편성]에서 파티를 짜고 스테이지를 골라 전투를 시작하세요.',
