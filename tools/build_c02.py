# ============================================================
# build_c02.py — 카시안(c02) 전투 원화 13장 변환 (build_c01 파생)
#
#   assets/sprites/_source_c02/*.png  →  assets/sprites/c01/*.png (192×128)
#
...
SRC = os.path.join(ROOT, 'assets', 'sprites', '_source_c02')
OUT = os.path.join(ROOT, 'assets', 'sprites', 'c02')

W, H = 192, 128
FOOT_ROW = 120
CHAR_H = 110            # 검사(58)보다 소폭 작게 — 지팡이·보주가 위로 길어 58이면 상단 잘림
THICK = 8
MIN_CC = 40            # 이보다 작은 잉크 조각은 배경 노이즈로 제거

# 오른쪽을 보는 원본 — 왼쪽으로 반전 (줌 시트 육안 + 출력 검수로 확정)
FLIP = set(['idle0', 'idle1', 'walk0', 'walk1', 'walk2', 'walk3',
            'attack0', 'attack1', 'attack2', 'attack3', 'cast', 'hurt', 'down'])   # 이 세트는 전원 오른쪽 보기 (얼굴 시트 실측)

# 지팡이가 왼쪽으로 길게 뻗는 프레임 — 발 중심 정렬에서 오른쪽으로 밀어 잘림 방지
OFFX = {'attack0': 12, 'attack2': 24}
