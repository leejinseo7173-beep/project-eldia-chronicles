...
export function autoAction(state, unit) {
  ...
  // 버티기 생존 본능: 이길 필요가 없는 전투에서 위험하면 웅크린다 (공격하면 반격을 맞는다).
  // 단, 한 방에 잡을 수 있는 적이 보이면 그건 잡는다 — 적 수를 줄이는 게 생존에 더 낫다.
  if (state.victory?.type === 'survive' && unit.hp <= unit.stats.maxHp * BALANCE.auto.surviveGuardHpPct) {
    const basic = SKILLS[unit.skills.basic];
    const nums = BALANCE.skills[unit.skills.basic] ?? {};
    const killable = L.validTargets(state, unit, basic).find((t) => basic.dmg
      && L.computeDamage(state, unit, t, { coef: nums.coef ?? 1, magic: !!basic.magic }) >= t.hp + t.shield);
    if (killable) return { type: 'skill', skillId: unit.skills.basic, targetId: killable.id };
    return { type: 'skip' };
  }
  ...
  // 성물은 대상 수 게이트 없음 — 만충이면 쏜다 (보스 단독에도. 게이트는 canUseSkill의 게이지)
  const sacredId = unit.skills.sacred && usable(state, unit, unit.skills.sacred) ? unit.skills.sacred : null;
