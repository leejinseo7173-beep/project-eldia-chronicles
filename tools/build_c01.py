# ============================================================
# build_c01.py — 브란(c01) 전투 원화 13장 변환 (v2 창+방패, c02 개선 파이프라인)
#
#   assets/sprites/_source_c01/*.png  →  assets/sprites/c01/*.png (192×128)
#
...
SRC = os.path.join(ROOT, 'assets', 'sprites', '_source_c01')
OUT = os.path.join(ROOT, 'assets', 'sprites', 'c01')

W, H = 288, 128          # 창이 길어 가로 확장 (세로 64·발끝 60 규격은 동일, 인물은 가로 중앙)
FOOT_ROW = 120
CHAR_H = 116            # 검사와 동일 — 브란은 덩치 기사 (창 수직 포즈는 잘림 검수)
THICK = 8
MIN_CC = 40            # 이보다 작은 잉크 조각은 배경 노이즈로 제거

# 오른쪽을 보는 원본 — 왼쪽으로 반전 (줌 시트 육안 + 출력 검수로 확정)
# v2 세트도 카시안처럼 전원 오른쪽 보기 — 인게임 실측으로 확정 (대기 방향 반대 신고, 2026-08-22).
# 얼굴 판별 요령: 살색 픽셀 뭉치가 머리 실루엣의 어느 쪽에 있는가 (섬네일 눈대중 금지)
FLIP = set(['idle0', 'idle1', 'walk0', 'walk1', 'walk2', 'walk3',
